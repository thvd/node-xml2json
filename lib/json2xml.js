...
ToXml.prototype.openTag = function(key) {
    this.completeTag();
    if (this.options.newLine !== false && this.countOpenParents > 0) {
        if (this.xml.substring(this.xml.length - this.options.newLine.length, this.xml.length) !== this.options.newLine) {
            this.xml += this.options.newLine;
        }

        for (var i = this.countOpenParents; i > 0; i--) {
            this.xml += this.options.indent;
        }
    }
    this.xml += '<' + key;
    this.tagIncomplete = true;
    this.countOpenParents += 1;
}
...
ToXml.prototype.closeTag = function(key) {
    ...
    if (this.options.newLine !== false) {
        this.xml += this.options.newLine;

        for (var i = this.countOpenParents - 1 /* because root doesn't need indent*/; i > 0; i--) {
            this.xml += this.options.indent;
        }
    }
